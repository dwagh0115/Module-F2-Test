let user =  `[{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
let Sort_az = document.getElementById("Sort_a-z");
let Sort_za = document.getElementById("Sort_z-a");
let SortMarks = document.getElementById("Sort_marks");
let SortPassing = document.getElementById("Sort_passing");
let Sortclass = document.getElementById("Sort_class");
let SortGender = document.getElementById("Sort_gender");
let product = document.getElementById("product");
let head = document.getElementById("head");
 
let mItem = document.createElement("tr");
  mItem.innerHTML = `
    <th> Id </th>
    <th> Name </th>
    <th> Gender </th>
...
function loadData(data){
 ...
 data.map((item) => {
    ...
    tItem.innerHTML = `
    <td> ${item.id} </td>
    <td> <img src="${item.img_src}" height="30" width="30"> ${item.first_name} ${item.last_name} </td>
    <td> ${item.gender} </td>
    <td> ${item.class} </td>
    <td> ${item.marks} </td>
    ...
    <td> ${item.email} </td>
    `
    product.append(tItem);

 })


}
Sort_az.addEventListener("click", sortAZ);
Sort_za.addEventListener("click", sortZA);
...
function sortAZ(){
    let sortednames = student.sort((a,b) => {
        if(a.first_name < b.first_name) return -1;
        else if(a.first_name > b.first_name) return 1;
        else return 0;
      });
      loadData(sortednames)
}
function sortZA(){
    let sortednames = student.sort((a,b) => {
        if(a.first_name < b.first_name) return 1;
...
function SortByMarks(){
    let sortedmarks = student.sort((a,b) => {
      if(a.marks < b.marks) return -1;
      else if(a.marks > b.marks) return 1;
      else return 0;
    });
    loadData(sortedmarks)

}
function SortByPassing(){
    let passingData = student.filter((item) => {
        return item.passing;
    })
...
function SortByClass(){
    let sortedclass = student.sort((a,b) => {
        if(a.class < b.class) return -1;
        else if(a.class > b.class) return 1;
        else return 0;
      });
      loadData(sortedclass)
}
...
searchBtn.addEventListener("click" , Email);
function Email(){
    let s = searchInput.value;
    let matchedItems = student.filter((item) =>{
